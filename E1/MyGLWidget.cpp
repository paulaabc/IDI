...
void MyGLWidget::modelTransformBracDret()
{
  ...
  TG = glm::rotate(TG,float((-angleg*M_PI)/180),glm::vec3(0.0,0.0,1.0));
  TG = glm::scale(TG,glm::vec3(0.1/0.5,0.4/2,1.0));
...
void MyGLWidget::modelTransformCorona()
{
  ...
  TG = glm::scale(TG,glm::vec3(0.3/1.5,0.4/2,0.0));
  glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]); 
}
